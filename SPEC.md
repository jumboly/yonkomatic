...
- `content/` — 動作確認用のサンプル素材 (`prompt.md`, `images/`, `sample-scenario.yaml`)。fork 先で利用者が自分の作品に書き換える
- `config.yaml` — デフォルト設定
- `.github/workflows/` — ワークフロー定義 (cron schedule はテンプレ側でコメントアウト)
- `.gitattributes` — `content/* merge=ours` で fork 先のカスタムを upstream 更新から保護
...
├── content/                         # ★ 作品素材 (動作確認用サンプル同梱、fork 先で利用者が書き換え)
│   ├── README.md                    # カスタマイズ手順
│   ├── prompt.md                    # キャラ / 世界観 / 画風を 1 ファイルに
│   ├── sample-scenario.yaml         # `test panel` 等の検証用シナリオ
│   └── images/                      # キャラ参考画像 (再帰 glob、サブディレクトリ自由)
│       ├── 01-*.png
│       └── README.md
...
- `yonkomatic test panel --scenario content/sample-scenario.yaml`
...
- `content/`: 動作確認用サンプル素材一式 (`prompt.md`, `images/`, `sample-scenario.yaml`)
- `LICENSE`: MIT 推奨
- `CONTRIBUTING.md`
- ユニットテスト（API はモック）
...
- `content/` の動作確認用サンプル素材 (`prompt.md`, `images/*.png`, `sample-scenario.yaml`)
- ドキュメント、ライセンス
- `config.yaml` のデフォルト
