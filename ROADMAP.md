...
- **完了**: Step 1, Step 2, Step 3, Step 4, **Step 5 (5a/5b/5c/5d 全て + simplify)**, **Step 5e (実装 + A/B 検証完了、AI 描画は本番採用見送り)**
- **次**: Step 6 (OSS 公開準備)
- **ブロッカー**: なし

最終更新: 2026-05-09 (Step 5e 検証完了、デフォルトは pil_overlay 維持)
...
- **`text_rendering.mode = model_render` は実験的**: Step 5e の検証で Flash + 7 話で完全一致 29% / 致命的バグ 14% を観測。本番運用には許容できないためデフォルトは `pil_overlay` 維持。CLI で `--text-mode model_render` を opt-in した場合は出力を必ず人手チェックすること。プロンプト強化で否定文 (重複・挿入禁止) を増やすと逆に LLM がそれらを誘発するので調整は慎重に。
