...
- **完了**: Step 1, Step 2, Step 3, Step 4, Step 5a, Step 5b, Step 5c (main 上のコード作業)
- **次**: Step 5d — 利用者ブランチで `daily-publish.yml` を `workflow_dispatch` 手動実行 + 7 日連続 cron 観察 → `/simplify` を 1 回 → ROADMAP.md 完了マーク
- **ブロッカー**: なし (5a-5c のコード変更は main 上で完了。実 GHA 検証のみ残)

最終更新: 2026-05-09 (Step 5a-5c 完了)
...
### ⏳ Step 5 — 日本語テキストオーバーレイ (composer fallback 本実装)
...
サブステップ進捗 (`/simplify` は Step 5 全体完了後 1 回):

- ✅ **Step 5a — Gemini プロンプト変更 + Dialogue.kind 追加** (commit `550edc6`)
...
- ⏳ **Step 5d — live ブランチ実 cron / `workflow_dispatch` 検証** (次)
  - main → live ブランチへ merge
  - live で `daily-publish.yml` を `workflow_dispatch` 1 回手動実行 → cache miss 経路 + Install fonts + 投稿成功を確認
  - 翌日 cron 自動実行で `cache-hit: true` になり Install fonts スキップを確認
  - 7 日連続稼働で 7 話とも文字化けゼロを Slack / GitHub Pages の両方で目視
  - `/simplify` を Step 5 全体に対して 1 回回す
  - 完了したら本ファイルの「現在地」を Step 6 へ進める
...
- **2026-05-09 (Step 5b 実装で確定)** `Dialogue.kind` を `Literal["speech","thought","shout"]` で追加 (デフォルト "speech")。`config.text_rendering.bubble_style` は `kind="speech"` の場合のみ参照され、`thought` は雲形・`shout` は星型バーストに固定される。kind 拡張時 (e.g. "shout-soft") は `panel/composer.py` の `_draw_one_bubble` 分岐に追加し、generator SYSTEM_PROMPT も更新する。
